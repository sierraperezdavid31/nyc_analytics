...
WITH seating_types AS (
   SELECT 
        DISTINCT seating_interest_sidewalk AS seating_interest,
        CASE WHEN UPPER(approved_for_sidewalk_seating) = "YES" THEN TRUE ELSE FALSE END AS approved_for_sidewalk,
        CASE WHEN UPPER(approved_for_roadway_seating) = "YES" THEN TRUE ELSE FALSE END AS approved_for_roadway
